...
The browser adapter is for HTTPS browser/PWA origins only. Tauri desktop must add an installed-app OAuth PKCE bridge and registered loopback/custom redirect before Drive can be enabled there; do not register `tauri://` as a Google web origin or present the browser popup as a supported desktop flow. If web SQLite/WASM is enabled, deployment also needs COOP/COEP isolation headers; Google popup authentication has a separate COOP requirement, so validate the chosen headers with real OAuth before release.
