...
export async function readText(path: string) {
  return new File(root(), path).text();
}
...
export async function hashFile(path: string) {
  return ReactNativeBlobUtil.fs.hash(
    nativeFile(path).uri.replace("file://", ""),
    "md5",
  );
}
...
export function bookFileReady(book: Book) {
  return (
    ...
    (book.format === "pdf" || new File(root(), book.id, "ready").exists)
  );
}
export async function unpack(book: Book) {
  if (book.format !== "pdf")
    await safeUnzip(
      ...
      new Directory(root(), book.id, "content"),
    );
  new File(root(), book.id, "ready").write("1");
}
...
async function safeUnzip(source: File, target: Directory) {
  ...
  if (
    ...
    entries.reduce((sum, e) => sum + e.size, 0) > 2 * 1024 ** 3 ||
    entries.some(
      (e) => !safePath(e.path) || e.isEncrypted || e.size > 128 * 1024 ** 2,
    )
  )
    throw new Error(
      "This archive is encrypted, unsafe, or exceeds the extraction limit.",
    );
...
export async function importBook(
  ...
  try {
    const original = new File(directory, `original.${format}`);
    await ReactNativeBlobUtil.fs.cp(
      uri.replace("file://", ""),
      original.uri.replace("file://", ""),
    );
    ...
    if (format !== "pdf") {
      const content = new Directory(directory, "content");
      const entries = await safeUnzip(original, content);
      if (format === "cbz") {
        pages.push(
          ...entries
            .filter(
              (e) =>
                ...
                /\.(png|jpe?g|webp|gif)$/i.test(e.path),
            )
            .map((e) => e.path)
        ...
        const info = entries.find((e) => /(^|\/)ComicInfo\.xml$/i.test(e.path));
        if (info && info.size < 1024 * 1024) {
          const parsed = z
            .object({ ComicInfo: z.record(z.string(), z.unknown()) })
            .safeParse(parser.parse(await new File(content, info.path).text()));
          if (parsed.success) {
            title = text(parsed.data.ComicInfo.Title) || title;
            author = text(parsed.data.ComicInfo.Writer) || author;
            language = text(parsed.data.ComicInfo.LanguageISO);
            if (
              text(parsed.data.ComicInfo.Manga).toLowerCase() ===
              "yesandrighttoleft"
            )
              direction = "rtl";
          }
        ...
        const container = z
          .object({
            container: z.object({
              rootfiles: z.object({
                rootfile: z.union([
                  z.object({ "@full-path": z.string() }),
                  z.array(z.object({ "@full-path": z.string() })),
                ]),
              }),
            }),
          })
          .parse(
            parser.parse(
              await new File(content, "META-INF/container.xml").text(),
            ),
          );
        const paths = asArray(container.container.rootfiles.rootfile);
        const opfPath = z.object({ "@full-path": z.string() }).parse(paths[0])[
          "@full-path"
        ];
        ...
        const opf = z
          .object({
            package: z.object({
              ...
              manifest: z.object({ item: z.unknown() }),
              spine: z.object({
                itemref: z.unknown(),
                "@page-progression-direction": z.string().optional(),
              }),
            }),
          })
          .parse(parser.parse(await new File(content, opfPath).text())).package;
        const base = opfPath.includes("/")
          ? opfPath.slice(0, opfPath.lastIndexOf("/") + 1)
          : "";
        const items = asArray(opf.manifest.item).map((v) =>
          itemSchema.parse(v),
        );
        title = text(opf.metadata.title) || title;
        author =
          asArray(opf.metadata.creator).map(text).filter(Boolean).join(", ") ||
          author;
        language = text(opf.metadata.language);
        direction =
          opf.spine["@page-progression-direction"] === "rtl" ? "rtl" : "ltr";
        for (const ref of asArray(opf.spine.itemref)) {
          const key = z.object({ "@idref": z.string() }).parse(ref)["@idref"];
          const item = items.find((i) => i["@id"] === key);
          if (!item || !safePath(base + item["@href"]))
        ...
        if (coverItem && safePath(base + coverItem["@href"]))
          cover = `${id}/content/${base}${coverItem["@href"]}`;
      }
    } else {
      const handle = original.open();
      try {
        if (new TextDecoder().decode(handle.readBytes(5)) !== "%PDF-")
    ...
    const book = bookSchema.parse({
      ...
      updatedAt: now,
      revision: 1,
      device: (await repository.setting("device")) ?? "local",
      deletedAt: null,
      asset: {
        path: `${id}/original.${format}`,
        hash: await ReactNativeBlobUtil.fs.hash(
          original.uri.replace("file://", ""),
          "md5",
        ),
        bytes: original.size,
    ...
    new File(directory, "ready").write("1");
